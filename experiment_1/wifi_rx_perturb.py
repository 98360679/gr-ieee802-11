...
# RX USRP serial (fixed - dedicated receiver)
RX_USRP_SERIAL = "325F45B"


def prompt_for_config():
    """Prompt user for TX device ID at startup."""
    print("\n" + "="*60)
    print("WiFi RX - Adversarial Attack Pipeline")
    print("="*60)
    
    while True:
        try:
            device_id = int(input("Enter TX device ID (1-7): ").strip())
            if 1 <= device_id <= 7:
                break
            else:
...
def setup_output_directory(device_id):
    ...
    output_dir = f"/media/nghoselab/T9/Data/ota_processed/device_{device_id}"
    os.makedirs(output_dir, exist_ok=True)
    return output_dir


# Prompt for configuration BEFORE Qt app starts
DEVICE_ID = prompt_for_config()
OUTPUT_DIR = setup_output_directory(DEVICE_ID)
AF_BIN_PATH = os.path.join(OUTPUT_DIR, "af.bin")
AFTER_BIN_PATH = os.path.join(OUTPUT_DIR, "after.bin")
EQU_BIN_PATH = os.path.join(OUTPUT_DIR, "equ.bin")
...
class wifi_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"Wifi Rx - Capturing Device {DEVICE_ID}")
       ...
       ##################################################
        ...
        self.blocks_file_sink_1 = blocks.file_sink(gr.sizeof_gr_complex*64, AF_BIN_PATH, False)
        self.blocks_file_sink_1.set_unbuffered(False)
        self.blocks_file_meta_sink_0_0_0 = blocks.file_meta_sink(gr.sizeof_gr_complex*64, AFTER_BIN_PATH, samp_rate, 1, blocks.GR_FILE_FLOAT, True, 1000000, pmt.make_dict(), True)
        self.blocks_file_meta_sink_0_0_0.set_unbuffered(False)
        self.blocks_file_meta_sink_0_0 = blocks.file_meta_sink(gr.sizeof_gr_complex*1, EQU_BIN_PATH, samp_rate, 1, blocks.GR_FILE_FLOAT, True, 1000000, pmt.make_dict(), False)
