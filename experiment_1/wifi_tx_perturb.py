...
# USRP Device Inventory
DEVICE_INVENTORY = {
    1: "3259376",
    2: "3259368",
    3: "32593C4",
    4: "3263606",
    5: "3259373",  # Primary TX, SARP class 6, 5.29 GHz
    6: "325936A",
    7: "3256204",
}
...
def prompt_for_config():
    """Prompt user for USRP serial number and device ID at startup."""
    print("\n" + "="*60)
    print("WiFi TX Perturbed - Adversarial Attack Pipeline")
    print("TX Machine: /home/misty/research/adversarial/")
    print("="*60)
    
    # Display device inventory
    print("\nUSRP Device Inventory:")
    for dev_id, serial in DEVICE_INVENTORY.items():
        note = " (primary TX, SARP class 6)" if dev_id == 5 else ""
        print(f"  Device {dev_id}: {serial}{note}")
    
    # Prompt for serial number
    print("\n" + "-"*40)
    while True:
        serial_input = input("Enter USRP serial number (or device ID 1-7): ").strip()
        
        # Check if user entered device ID
        if serial_input.isdigit() and int(serial_input) in DEVICE_INVENTORY:
            device_id = int(serial_input)
            serial_number = DEVICE_INVENTORY[device_id]
            print(f"  -> Using Device {device_id}: serial {serial_number}")
            break
        # Check if user entered serial directly
        elif serial_input.upper() in DEVICE_INVENTORY.values():
            serial_number = serial_input.upper()
            device_id = [k for k, v in DEVICE_INVENTORY.items() if v == serial_number][0]
            print(f"  -> Matched Device {device_id}: serial {serial_number}")
            break
        else:
            print(f"  Warning: Serial '{serial_input}' not in inventory.")
            confirm = input("  Use anyway? (y/n): ").strip().lower()
            if confirm == 'y':
                serial_number = serial_input
                # Prompt for device ID for directory naming
                while True:
                    try:
                        device_id = int(input("  Enter device ID (1-7) for directory naming: ").strip())
                        if 1 <= device_id <= 7:
                            break
    ...
    return serial_number, device_id


def setup_output_directory(device_id):
    """Create data/device_N/ directory if it doesn't exist."""
    output_dir = f"data/device_{device_id}"
    os.makedirs(output_dir, exist_ok=True)
    print(f"\nOutput directory: {output_dir}/")
    return output_dir


# Prompt for configuration BEFORE Qt app starts
SERIAL_NUMBER, DEVICE_ID = prompt_for_config()
OUTPUT_DIR = setup_output_directory(DEVICE_ID)
TX_BIN_PATH = os.path.join(OUTPUT_DIR, "tx.bin")
print(f"TX output file: {TX_BIN_PATH}")
print("="*60 + "\n")
...
class wifi_tx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"Wifi Tx - Device {DEVICE_ID} ({SERIAL_NUMBER})")
        ...
        self.blocks_file_sink_0 = blocks.file_sink(
            gr.sizeof_gr_complex, TX_BIN_PATH, False)
    ...
    def send_dynamic_frame(self):
        ...
        try:
            current_size = os.path.getsize(TX_BIN_PATH)
